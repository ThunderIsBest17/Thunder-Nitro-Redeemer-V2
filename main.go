...
func (c *Redeemer) FetchCode(code string) (string, error) {
    ...
    req := fasthttp.AcquireRequest()
    req.SetRequestURI(fmt.Sprintf("https://discord.com/api/v9/entitlements/partner-promotions/%s", slices[5]))
...
func main() {
	...
	for len(duplicatedvcc) > 0 && len(tokenLines) > 0 && len(promoLines) > 0 {
		...
		go func(tokenfull, vcc, promo string) {
			...
			client.Promo, err = client.FetchCode(promo)
			err = client.Redeem()
			if err != nil {
